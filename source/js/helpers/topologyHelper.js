...
export default class topologyMapHelper {
    ...
    getLayers(map, topologyLayer, geoLayer, directory, typeId, stateToDisplay, markerEvents, pathEvents, _selectdPathObj) {
        ...
        var _parentTerminals = _selectdPathObj && _selectdPathObj.terminations;

        /*_parentTerminals && (topologyLayer.paths = topologyLayer.paths.filter((val,i)=>{
            var _term =val.id.split("link_")[1].split("#");
            return !(_term[0] ==_term[1] && (_term[1] == _parentTerminals[0] || _term[1] == _parentTerminals[1]) );
        }))*/
        let onlyCrossConnectedPath = [];
        if (_parentTerminals) {
            for (var i = 0; i < topologyLayer.paths.length; i++) {
                //console.log(topologyLayer.paths[i].id,splitId(topologyLayer.paths[i]))

                let splittedId = this.splitId(topologyLayer.paths[i])
                if (splittedId[0] == splittedId[1] && topologyLayer.paths[i - 1].id.indexOf(splittedId[1]) != -1) {
                    onlyCrossConnectedPath.push(topologyLayer.paths.splice(i, 1)[0]);
                    i--;
                    continue;
                }
            }
        ...
        var AvailableViewLength = viewLength - gPathsWidth - uPathsWidth;
        var pathWidth = AvailableViewLength / (topologyLayer.paths.length - gCount - uCount)
        ...
        this.drawTopologyLayer(map, topologyLayer, pathWidth, gPathWidth, uPathWidth, start, stateToDisplay, markerEvents, pathEvents, onlyCrossConnectedPath);

        var lMarkers = this.drawGeoLayer(geoLayer, topologyLayer, directory, stateToDisplay, markerEvents, pathEvents);
    ...
    drawGeoLayer(layer, topologyLayer, directory, stateToDisplay, markerEvents, pathEvents) {
        var layerGroup = layer.layers[layer.layers.length - 1];
        var lMarkers = [];
        if (topologyLayer.layers && topologyLayer.layers.length > 0) {
            var layers = topologyLayer.layers[topologyLayer.layers.length - 1].getLayers()

            //drawing markers
            var lMarkersFromTopology = layers && layers.filter((layer) => {
                return layer.options.type == "marker" || layer.options.type == "gMarker" || layer.options.type == "crossConnectMarker"
            })
            lMarkersFromTopology.forEach((markerClass) => {
                var geo_markerType = "marker";
                var ccPaths = [];
                if (markerClass.options.type == "crossConnectMarker") {
                    geo_markerType = "crossConnectMarker"
                    // xPaths = "link_" + markerClass.options.id + '#' + markerClass.options.id;
                    // xPaths = topologyMapHelper.directory[xPaths].siblings.concat(topologyMapHelper.directory[xPaths].id)
                    ccPaths = markerClass.options.ccPaths

                }
                var locationNodeName = directory[markerClass.options.id].location
                if (!directory[locationNodeName]) {

                    return false;
                }
                let latitude = directory[locationNodeName].latitude;
                let longitude = directory[locationNodeName].longitude;

                let markerObj = {
                    icon: this.drawMarkerV1(topologyMapHelper.directory[locationNodeName], geo_markerType, stateToDisplay, undefined, this.eqIcon),
                    isShowLabel: true,
                    id: directory[markerClass.options.id].location,
                    type: geo_markerType,
                    ccPaths
                }
                var marker = L.marker(L.latLng(latitude, longitude), markerObj).on('click', markerEvents.click)
                    .on('mousemove', markerEvents.enter)
                    .on('mouseout', markerEvents.leave)
                    .on('contextmenu', markerEvents.contextmenu);

                if (latitude == undefined || longitude == undefined || latitude == null || longitude == null) {
                    marker.options["invalidLocation"] = directory[markerClass.options.id].name;
                }
            ...
            //drawingPath
            var lPolylinesFromTopology = layers && layers.filter((layer) => {
                return layer.options.type == "path" || layer.options.type == "Path" || layer.options.type == "gPath" || layer.options.type == "uPath"
            })

            lPolylinesFromTopology.forEach((polylineClass) => {

                if (directory[polylineClass.options.id]) {
                    var pointList = directory[polylineClass.options.id].terminations.map((termination) => {
                        var locationNodeName = directory[termination].location
                        if (!directory[locationNodeName]) {

                            return false;
                        }
                        return L.latLng(directory[locationNodeName].latitude, directory[locationNodeName].longitude)
                    })
                    var path = directory[polylineClass.options.id] || {};
                    var disablePathBundleFlag = false;
                    var isPathBundle = false;

                    path.siblings = polylineClass.options.pathWithSibling;
                    if (path.siblings && path.siblings.length > 0) {
                        // for (var i = 0; i < path.siblings.length; i++) {
                        //     if (topologyMapHelper.directory[path.siblings[i]].layerDepth != topologyMapHelper.directory[polylineClass.options.id].layerDepth) {
                        //         //disable path bundle..
                        ...
                        //     }
                        // }
                        // isPathBundle = !path.selectedPathID && !disablePathBundleFlag;
                        isPathBundle = !disablePathBundleFlag;
                    }
                    else {
                        disablePathBundleFlag = false;
                    }
                    var polyline = new L.Polyline(pointList, {
                        className: "geoPath",
                        id: polylineClass.options.id,
                        type: polylineClass.options.type,
                        opacity: 1,
                        weight: polylineClass.options.weight,
                        color: polylineClass.options.color,
                        pathWithSibling: (path.siblings && path.siblings.length > 0) ? path.siblings : [],
                        // ring: !isPathBundle
                        disablePathBundleFlag
                    });

                    polyline.on("add", function (event) {

                        // var isPathBundleExists = layerGroup && layerGroup.getLayers().findIndex((layer) => {
                        //     return layer.options.type == "bundleIcon" && layer.options.id == event.target.options.id
                        // }) != -1
                        // if (isPathBundle && !isPathBundleExists) {
                        if (event.target.options.disablePathBundleFlag) {
                            var marker = L.marker(event.target.getCenter(), {
                                icon: topologyMapHelper.drawPathBundleIcon(topologyMapHelper.directory, path.siblings, stateToDisplay.path),
                                isShowLabel: true,
                                id: event.target.options.id,
                                type: "bundleIcon",
                            }).on('click', function (event) { }, this)
                            layerGroup.addLayer(marker)
                        }
                    })
            ...
            //drawingPathBundle
            var lBundleIconFromTopology = layers && layers.filter((layer) => {
                return layer.options.type == "bundleIcon"
            })

            lBundleIconFromTopology.forEach((bundleIconClass) => {
                // var pointList = directory[bundleIconClass.options.id].terminations.map((termination) => {
                //     return L.latLng(directory[termination].latitude, directory[termination].longitude)
                // })

                // var bundleIconPoint = { lat: pointList[0].lat + (terminatingPoints[1].lat - terminatingPoints[0].lat) / 2, lng: terminatingPoints[0].lng }
                // var marker = L.marker( L.latLng(bundleIconPoint.lat, bundleIconPoint.lng), {
                //     icon: bundleIconClass.options.icon,
                //     isShowLabel: bundleIconClass.options.isShowLabel,
                //     id: bundleIconClass.options.id,
                //     type: "bundleIcon",
                // }).on('click', function (event) {
                // }, this)
                // layerGroup.addLayer(marker)
    ...
    drawPaths(layer, index, map, terminatingPoints, stateToDisplay, markerEvents, pathEvents, onlyCrossConnectedPath) {
        ...
        path.terminations && path.terminations.forEach((markerID, terminationIndex) => {
            this.drawMarkers(layer, map, path, index, type, markerID, terminationIndex, terminatingPoints, markerEvents, stateToDisplay, onlyCrossConnectedPath)
        }, this)
    }

    drawLocationMarker(layer, map, directory, markerType = "marker", stateToDisplay, markerEvents) {
        console.log("inside drawlocationmarker");
        var layerGroup = L.layerGroup();
        map.addLayer(layerGroup);
        layer.layers.push(layerGroup);
        var lMarkers = [];
        Object.keys(directory).forEach(id => {
            if (directory[id] && directory[id].type.toLowerCase() == "location") {
                var marker = L.marker(L.latLng(directory[id].latitude, directory[id].longitude), {
                    icon: this.drawMarker(directory[id], markerType, stateToDisplay),
                    isShowLabel: true,
                    id: id,
                    type: markerType,
    ...
    drawMarkers(layer, map, path, pathIndex, pathType, markerID, terminationIndex, terminatingPoints, markerEvents, stateToDisplay, onlyCrossConnectedPath) {
        ...
        */
        let isCrossConnected = onlyCrossConnectedPath.some((val, i) => {
            return val.id.indexOf(markerID) > -1
        })
        ...
        */
        ...
        if (previousLayer && markerType != "gMarker") {
            ...
            if (lMarkerLayerWithSameID.length > 0) {
                ...
                // markerType = isCrossConnected ? "crossConnectMarker" : markerType
                previousLayerMarker.setIcon(this.drawMarker(topologyMapHelper.directory[previousLayerMarker.options.id], markerType, stateToDisplay, false))
        ...
        var marker = L.marker(markerPoints, {
            icon: this.drawMarker(topologyMapHelper.directory[markerID], markerType, stateToDisplay),
            isShowLabel: true,
            id: markerID,
            type: markerType,
        }).on('click', markerEvents.click)
    ...
    static drawMarkerCopy(itemFromDirectory, stateToDisplay) {
            ...
            if (stateFromDir && stateFromDir[state]) {
                ...
                if (filteredSubState) {
                    ...
                    if (stateIconColor.badge) {
                        var stateBadge = {
                            color: stateIconColor.color,
                            badge: stateIconColor.badge
                        }
                    } else
        ...
        return new L.DivIcon({
            ...
            html: "<div id='mapviewmarkercontainer'>\
                    " + equipmentMapHelper.drawSVG({
                value: "marker",
                color: stateMarkerColor ? stateMarkerColor : "#acacac",
                state: stateBadge,
                eqIcon:eqIcon, details:itemFromDirectory
            }) + "\
    ...
    drawMarker(itemFromDirectory, type = 'marker', stateToDisplay, isShowLabel = true) {
        ...
        let eqIcon=this.tMHelperObj.eqIcon;
        return new L.DivIcon({
            className: isShowLabel ? 'showLabel' : '',
    ...
    updateLayers(layers, directory, stateToDisplay, drawMarker, typeid) {
        ...
        if (typeid == '2') {
                ...
                if (typeof layers[ind].eachLayer === "function") {
                    ...
                    layers[ind].eachLayer((layer) => {
                        if (typeid == 2 && layer.options.UUID) {
                            ...
                            layer.setIcon(drawMarker(value[0], "marker", stateToDisplay, layer.options.isShowLabel));
                            ...
                            layer.setIcon(drawMarker(value[0], layer.options.type, stateToDisplay, layer.options.isShowLabel))
            ...
            layers.forEach((layer) => {
                console.log("inside each layer of layers..");
                layer.eachLayer((layer) => {
                    ...
                    if (layer.options.type == "marker" || layer.options.type == "gMarker" || layer.options.type == "crossConnectMarker") {
                        ...
                        layer.setIcon(drawMarker(value, layer.options.type, stateToDisplay, layer.options.isShowLabel))
  ...
  <g id="svg_1">
   <svg id="svg_3" y="10%" x="10%" height="80%" width="80%">
    ...
    getLayersV1(map, topologyLayer, geoLayer, directory, typeId, stateToDisplay, markerEvents, pathEvents, _selectdPathObj, handleCustomError) {
        ...
        // } 
        // else {
        //     //path, isRootPathMarker is not used 
        ...
        //     var viewLength = window.innerWidth / 2;
        //     var terminatingPoints = [{ x: viewLength, y: 150 }];
        //     var markerID = rootPath.terminations[0];
        //     var type = "marker";
        //     var index = 0;
        //     var terminationIndex = 0;
        //     var path = rootPath;
        //     this.drawMarkersV1(topologyLayer, map, path, index, type, markerID, terminationIndex, terminatingPoints, markerEvents, stateToDisplay, [rootPath.id], true);
        // }
        ...
        if (typeId == 0)
            ...
            lMarkers.forEach(function (val, j) {
                if (val.options.invalidLocation)
                    invalidEquipementLocation.push(val.options.invalidLocation);
            })
    ...
    drawMarkersV1(layer, map, path, pathIndex, pathType, markerID, terminationIndex, terminatingPoints, markerEvents, stateToDisplay, ccPaths, isRootPathMarker) {
        ...
        /*
            ...
            * For termination 2 to be gMarker, check if no next cNode or next cNode is gPath/uPath 
        */
        // let isCrossConnected = ccPaths.some((val, i) => {
        //     let _CCEndPoint1= topologyMapHelper.directory[val.id].terminations[0];
        //     let _CCEndPoint2= topologyMapHelper.directory[val.id].terminations[1];
        ...
        */
        ...
        if (previousLayer && markerType != "gMarker") {
            ...
            if (lMarkerLayerWithSameID.length > 0) {
                ...
                var previousLayerMarker = isMarkerLayerAlreadyExists ? (lMarkerLayerWithSameID[1] || lMarkerLayerWithSameID[0]) : lMarkerLayerWithSameID[0];
                previousLayerMarker.options.isShowLabel = false;
                // markerType = crossConnectedAr ? "crossConnectMarker" : markerType
        ...
        var marker = L.marker(markerPoints, {
            icon: this.drawMarkerV1(topologyMapHelper.directory[markerID], markerType, stateToDisplay, undefined, this.eqIcon),
            isShowLabel: true,
            id: markerID,
            type: markerType,
    ...
    drawMarkerV1(itemFromDirectory, type = 'marker', stateToDisplay, isShowLabel = true, eqIcon) {
        //  var markerStateToDisplay = stateToDisplay || {};
        var statesObjFromStatusPanel = topologyMapHelper.filteredState(itemFromDirectory, stateToDisplay.marker)
        for (var i = 0; i < statesObjFromStatusPanel.length; i++) {
            var stateObjFromStatusPanel = statesObjFromStatusPanel[i];
