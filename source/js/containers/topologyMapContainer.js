...
class TopologyMapRoot extends Map {
    ...
    //             // if (selectedPathID && rootPath.siblings && rootPath.siblings.length > 0) {
    //             if (selectedPathID ) {
    ...
    //                 topologyMapHelper.maxLayer = Math.max(...lLayerDepth)
    //                 self.changeSeekBar(self.topologyMap.layers.length, topologyMapHelper.maxLayer)
    //             }
    //         }
    //     }
    // }
    ...
    updatePopOverProp(event) {
        ...
        if (typeId == 2) {
            let entity = topologyMapHelper.directory[entityID];
            let self = this;
            if (entity && entity.type.toLowerCase() == "location") {
                var associations = entity.cNodes && entity.cNodes.map((cNode) => {
                    return self.getPopOverEntity(topologyMapHelper.directory[cNode], stateToDisplay[type])
                })
                propData.associations = associations
            }
        }
    ...
    retainPathState(layer, directory, stateToDisplay, drawMarker) {
        ...
        layer.layers[layer.layers.length - 1].eachLayer((layer) => {
            ...
            if (layer.options.type && layer.options.type == "marker") {
                ...
                layer.setIcon(drawMarker(directory[layer.options.id], "marker", stateToDisplay))
            }
        })
    ...
    expandLayer() {
        ...
        var paths = [];
        var _selectdPathObj;
        this.topologyMap.paths.forEach((pathFromLayerObj) => {
            //  console.log("inside each path");
                    ...
                    if (path.selectedPathID) {
                        ...
                        dir.forEach(function (item, index) {
                            if (item.type.toLowerCase() == 'path') {
                                ...
                                if (selectedPath[0].siblings[0] == item.id) {//&& selectedPath[0].cNodes.includes(item.id.replace("_CLONE_1", ""), 0)){
                                    ringparentItem = item;
                        ...
                        var endIndexOfRing = paths.findIndex(x => x.id === sequence[sequence.length - 2].id);
        ...
        if (pathExpanded) {
            ...
            if (lPathsID.length > 0) {
                console.log(this.topologyMap.paths);
                this.removeExistingLayersFromMap(this.geoMap.layers, this.map);
                this.tMHelperObj.getLayers(this.map, this.topologyMap, this.geoMap, topologyMapHelper.directory, typeId, stateToDisplay, this.markerEvents.call(this), this.pathEvents.call(this), _selectdPathObj);
            } else {
    ...
    collapseLayer() {
        ...
        if (this.topologyMap.layers.length > 1) {
            ...
            this.topologyMap.paths = this.retainPathState(this.topologyMap, topologyMapHelper.directory, stateToDisplay, this.tMHelperObj.drawMarker.bind(this))
    ...
    collapsePath(selectedPathID) {
        ...
        if (selectedPathID && selectedPathID != "") {
            ...
            if (this.topologyMap.layers.length > 0) {
                ...
                this.topologyMap.paths = this.retainPathStateV1(this.topologyMap, topologyMapHelper.directory, stateToDisplay, this.tMHelperObj.drawMarker.bind(this))
 ...
 axios.all(promises).then(results => {
    ...
    componentDidMount() {
        ...
        if (data.length > 0 && !topologyMapHelper.directory && rootPathID) {
            ...
            if (typeId == 2) {
                this.tMHelperObj.drawLocationMarker(this.geoMap, this.map, topologyMapHelper.directory, "marker", stateToDisplay, this.markerEvents.call(this))
                this.tMHelperObj.getLayers(this.map, this.topologyMap, this.geoMap, topologyMapHelper.directory, typeId, stateToDisplay, this.markerEvents.call(this), this.pathEvents.call(this));
            } else {
...
var customerrorMessage;
        ...
        if (statusUpdateTimeStamp && statusUpdateTimeStamp != this.props.statusPanel.statusUpdateTimeStamp) {
            ...
            if (nextProp.typeId == 2) {
                ...
                this.tMHelperObj.updateLayers(this.map._layers, topologyMapHelper.directory, nextProp.stateToDisplay, this.tMHelperObj.drawMarker.bind(this), nextProp.typeId)
            } else {
                this.tMHelperObj.updateLayers([].concat(this.topologyMap.layers, this.geoMap.layers), topologyMapHelper.directory, nextProp.stateToDisplay, this.tMHelperObj.drawMarker.bind(this), nextProp.typeId)
            }
        ...
        if (!topologyMapHelper.directory && !Array.isArray(data)) {
            ...
            if (typeId == 2) {
                ...
                // this.tMHelperObj.getLayers(this.map, this.topologyMap, this.geoMap, topologyMapHelper.directory, typeId, stateToDisplay, this.markerEvents.call(this), this.pathEvents.call(this));
                // 
                this.tMHelperObj.getLayersV1(this.map, this.topologyMap, this.geoMap, topologyMapHelper.directory, typeId, stateToDisplay, this.markerEvents.call(this), this.pathEvents.call(this), rootPathLinkID, this.handleCustomError);
            ...
            if (typeId == 2) {
                topologyMapHelper.directory = topologyMapHelper.directory || {}
                topologyMapHelper.directory = data;
                if (data.length > 0) {
                    topologyMapHelper.directory = data.reduce(function (total, currentValue, i) {
                        total[currentValue.id] = currentValue
                    ...
                    //concat with existing value based on condition:
                    if (nextProp.data.length != this.props.data.length) {
                        this.markerClusters = this.tMHelperObj.getMarkers(topologyMapHelper.directory, stateToDisplay, onMarkerClick, nextProp);
                        this.map.addLayer(this.markerClusters);
                        var directory = topologyMapHelper.directory;
                        topologyMapHelper.directory = Object.assign(topologyMapHelper.directory, directory)
                    }

                }
                if (typeId == '2') {
                    var locMissing = false;
                    for (var dataIndex = 0; dataIndex < data.length; dataIndex++) {
                        if (data[dataIndex].type.toLowerCase() == 'location' && (!data[dataIndex].latitude || !data[dataIndex].longitude)) {
                            locMissing = true;
                        } else {
                ...
                if (data.length > 0 && data[0].terminations && data[0].terminations.latitude && data[0].terminations.longitude) {
                    //this.map.setView([data[0].terminations.latitude, data[0].terminations.longitude], 15);
                    this.tMHelperObj.drawLocationMarker(this.geoMap, this.map, topologyMapHelper.directory, "marker", stateToDisplay, this.markerEvents.call(this));
                } else if (data.length > 0 && data[0].latitude && data[0].longitude) {
                    //this.map.setView([data[0].latitude, data[0].longitude], 15);
                    if (!statusUpdateTimeStamp && !this.props.statusPanel.statusUpdateTimeStamp) {
                        this.map.setView([data[0].latitude, data[0].longitude], 15);
                    }
    ...
    expandLayerV1() {
        ...
        if(topologyMapHelper.directory.selectedPathID){
        ...
        if (pathExpanded) {
            ...
            if (lPathsID.length > 0) {
                this.removeExistingLayersFromMap(this.geoMap.layers, this.map);
                this.tMHelperObj.getLayersV1(this.map, this.topologyMap, this.geoMap, topologyMapHelper.directory, typeId, stateToDisplay, this.markerEvents.call(this), this.pathEvents.call(this), _selectedP,this.handleCustomError);
            } else {
                //errorMessage:no layers to expand
            }
    ...
    retainPathStateV1(layer, directory, stateToDisplay, drawMarker) {
        //Changing layer.path state to previous layer
        var path = [];
        layer.layers[layer.layers.length - 1].eachLayer((layer) => {
